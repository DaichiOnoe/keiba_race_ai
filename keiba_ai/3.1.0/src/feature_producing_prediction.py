...
class PredictionFeatureCreator:
    ...
    def fetch_shubuta_page_html(self, race_id: str) -> None:
        ...
        self.htmls[race_id] = urlopen(req).read()

        return self.htmls[race_id]
    ...
    def fetch_results(
        ...
        for a in a_list:
            ...
            # jockey_id = re.findall(r"[a-zA-Z0-9]+", a["href"])[0]
            jockey_id_match = re.findall(r"\d{5}", a["href"])  # 数字5桁を抽出
            if jockey_id_match:  # 一致する場合
                jockey_id = int(jockey_id_match[0])
            else:  # 一致しない場合
                jockey_id = np.nan
        ...
        # 前処理
        df = df[(df.iloc[:, 2] != "取消") & (df.iloc[:, 2] != "除外")]
        # df = df[df.iloc[:, 2] != "取消"]
        ...
        if self.weight:
            columns.extend(["weight", "weight_diff"])
    ...
    def create_features(
        ...
        self.agg_interval()
        # 各種テーブルの取得
        self.fetch_shubuta_page_html(race_id = self.race_id)
        self.fetch_results(race_id = self.race_id, html = self.htmls[self.race_id])
        self.fetch_race_info(race_id = self.race_id, html = self.htmls[self.race_id])
    ...
    async def update_odds_and_popularity(
            self,
            )-> pd.DataFrame:
        """"
        "単勝オッズと人気をJRAの公式サイトから取得し、更新する関数。"
        """
        place_mapping = {
            1: '札幌',
            2: '函館',

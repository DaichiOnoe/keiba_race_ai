...
def process_results(
    ...
    df["weight_diff"] = pd.to_numeric(df["馬体重"].str.extract(r"\((.+)\)")[0], errors="coerce").fillna(0).astype(int)
...
def process_horse_results(
    ...
    df["course_len"] = df["距離"].str.extract(r"(\d+)").astype(float)
...
def process_return_table(
    POPULATION_DIR: Path = POPULATION_DIR,
    POPULATION_FILENAME: str = "population_5year.csv",
    input_dir: Path = RAWDF_DIR,
    output_dir: Path = OUTPUT_DIR,

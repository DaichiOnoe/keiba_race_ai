...
class FeatureCreator:
    ...
    def create_baselog(self, chunk_size: int = 10000):
        """
        ...
        for chunk in tqdm(chunks, desc="Processing population chunks"):
            baselog_chunk = (
                chunk.merge(
                    self.horse_results, on="horse_id", suffixes=("", "_horse")
                )
            ...
            baselog_list.append(baselog_chunk)

        # チャンクを結合してbaselogを作成
        self.baselog = pd.concat(baselog_list, ignore_index=True)
    ...
    def agg_horse_per_group_cols(
        ...
        # for chunk in tqdm(chunks, desc=f"Creating baselog for {df_name}"):
        #     baselog_chunk = (
        #         chunk.merge(
        #             self.race_info[["race_id"] + group_cols], on="race_id"
        #         )
        ...
        # チャンクを結合してbaselogを作成
        # baselog = pd.concat(baselog_list, ignore_index=True)
        ...
        # # チャンクに分割
        # chunks = [baselog[i:i + chunk_size] for i in range(0, len(baselog), chunk_size)]
        # merged_df = self.population.copy()

        # for chunk in tqdm(chunks, desc=f"Processing chunks for {df_name}"):
        #     grouped_df = chunk.groupby(["race_id", "horse_id"])
        ...
        #         relative_df = ((df - tmp_df.mean()) / tmp_df.std()).add_suffix("_relative")
        #         merged_df = merged_df.merge(
        #             relative_df, on=["race_id", "horse_id"], how="left", suffixes=("", "_new")
        #         )
        # self.agg_horse_per_group_cols_dfs[df_name] = merged_df
    ...
    def agg_jockey(self):
        ...
        df["year"] = pd.to_datetime(df["date"]).dt.year - 1
           # `jockey_leading` の `jockey_id` と `year` の組み合わせがユニークであることを保証
        self.jockey_leading = self.jockey_leading.drop_duplicates(subset=["jockey_id", "year"])
        df = (
    ...
    def agg_trainer(self):
        ...
        df["year"] = pd.to_datetime(df["date"]).dt.year - 1
        self.trainer_leading = self.trainer_leading.drop_duplicates(subset=["trainer_id", "year"])
        df = (
    ...
    def create_features(self) -> pd.DataFrame:
        ...
        for chunk in chunks:
            ...
            features_chunk = (
                chunk.merge(self.results, on=["race_id", "horse_id"])
                .merge(self.race_info, on=["race_id", "date"])
                .merge(
                    self.agg_horse_n_races_df,
                    on=["race_id", "date", "horse_id"],
